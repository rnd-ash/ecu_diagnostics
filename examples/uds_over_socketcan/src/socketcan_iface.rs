...
use socketcan_isotp::{EFF_FLAG, FlowControlOptions, IsoTpBehaviour, IsoTpOptions, IsoTpSocket};
...
impl BaseChannel for SocketCANInterface {
    fn open(&mut self) -> ecu_diagnostics::channel::ChannelResult<()> {
        if self.opts.is_none() {
            return Err(ChannelError::APIError { api_name: "SocketCAN".into(), code: 0, desc: "IsoTP configuration is null".into() })
        }
        if self.iface.is_some() {
            return Err(ChannelError::APIError { api_name: "SocketCAN".into(), code: 1, desc: "SocketCAN interface is already open".into() })
        }
        ...
        match IsoTpSocket::open_with_opts(
            ...
            Err(e) => {
                return Err(ChannelError::APIError { api_name: "SocketCAN".into(), code: 1, desc: format!("{}", e) })
            }
        }
    }
...
impl IsoTPChannel for SocketCANInterface {
    fn set_iso_tp_cfg(&mut self, cfg: IsoTPSettings) -> ecu_diagnostics::channel::ChannelResult<()> {
        self.opts = Some(cfg);
        Ok(())
    }
}