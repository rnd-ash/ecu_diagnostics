...
pub fn read_stored_dtcs_iso15031(server: &mut Kwp2000DiagnosticServer, range: DTCRange) -> DiagServerResult<Vec<DTC>> {
...
pub fn read_supported_dtcs_iso15031(server: &mut Kwp2000DiagnosticServer, range: DTCRange) -> DiagServerResult<Vec<DTC>> {
    let mut res: Vec<DTC> = Vec::new();
    
    loop {
        let res_bytes = server.execute_command_with_response(KWP2000Command::ReadDiagnosticTroubleCodesByStatus, &range.to_args(0x01))?;
...
pub fn read_stored_dtcs(server: &mut Kwp2000DiagnosticServer, range: DTCRange) -> DiagServerResult<Vec<DTC>> {
    let mut res = server.execute_command_with_response(KWP2000Command::ReadDiagnosticTroubleCodesByStatus, &range.to_args(0x02))?;
    if res.len() < 5 { // No DTCs stored
        return Ok(Vec::new())
    }
    let num_dtcs = res[1];
...
pub fn read_supported_dtcs(server: &mut Kwp2000DiagnosticServer, range: DTCRange) -> DiagServerResult<Vec<DTC>> {
    let mut res: Vec<DTC> = Vec::new();
    loop {
        let mut res_bytes = server.execute_command_with_response(KWP2000Command::ReadDiagnosticTroubleCodesByStatus, &range.to_args(0x03))?;
        
        if res_bytes.len() < 5 { // No DTCs stored
            return Ok(Vec::new())
        }
        res_bytes.drain(0..2); // Remove everything up to the first DTC
        if res_bytes.len() % 3 != 0 { // Each DTC is 3 bytes, so this should divide by 0 if ECU response is valid
            return Err(DiagError::InvalidResponseLength)
        }
...
pub fn read_extended_supported_dtcs(server: &mut Kwp2000DiagnosticServer, range: DTCRange) -> DiagServerResult<u16> {
    match server.execute_command_with_response(KWP2000Command::ReadDiagnosticTroubleCodesByStatus, &range.to_args(0xE0)) {
        Ok(x) => {
            if x.len() == 3 {
