...
pub fn request_seed(server: &mut Kwp2000DiagnosticServer, access_mode: u8) -> DiagServerResult<Vec<u8>> {
    if access_mode % 2 == 0 {
        return Err(DiagError::ParameterInvalid)
    }
    let mut res = server.execute_command_with_response(KWP2000Command::SecurityAccess, &[access_mode])?;
...
/// should be THE SAME as what was provided to [request_seed]
pub fn unlock_ecu_with_key(server: &mut Kwp2000DiagnosticServer, access_mode: u8, key: &[u8]) -> DiagServerResult<()> {
    if access_mode % 2 == 0 {
        return Err(DiagError::ParameterInvalid)
    }
    let mut args = vec![access_mode+1];
    args.extend_from_slice(key);
    server.execute_command_with_response(KWP2000Command::SecurityAccess, &args).map(|_|())
}