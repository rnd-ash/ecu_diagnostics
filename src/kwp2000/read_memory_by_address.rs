...
pub fn read_memory(server: &mut Kwp2000DiagnosticServer, address: u32, size: u8) -> DiagServerResult<Vec<u8>> {
    server.execute_command_with_response(KWP2000Command::ReadMemoryByAddress, &[
        (address >> 16) as u8,
        (address >> 8) as u8,
        address as u8,
        size
