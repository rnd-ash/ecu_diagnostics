...
use crate::channel::{
    CanChannel, CanFrame, ChannelError, ChannelResult, Packet, PacketChannel, PayloadChannel,
    VwTp2Settings,
};
...
impl<T: VwApplicationProtocol> VwTransport2Channel<T> {
    ...
    fn start_background_thread(
        &mut self,
        mut bs: u8,
        mut ack_timeout: Duration,
        mut inter_packet_ms: Duration,
        settings: VwTp2Settings,
    ) {
                    ...
                    match data[0] & 0xF0 {
                        0x00 => {
                            ...
                            if let ThreadState::MultiRx { .. } = &mut state {
                                ...
                                log::error!(
                                    "Received request for ack but aren't receiving anything?"
                                )
                            }
                        }
                        0x10 => {
                            if state == ThreadState::Idle {
                                ...
                                if packet_id == *expected_pci {
                                    ...
                                    log::error!(
                                        "Expected packet ID {}, got {packet_id}",
                                        *expected_pci
                                    );
                                    state = ThreadState::Idle;
                                }
                        ...
                        0x20 => {
                            ...
                            if state == ThreadState::Idle {
                                ...
                                state = ThreadState::MultiRx {
                                    expected_pci: (packet_id + 1) & 0x0F,
                                    buf: data[1..].to_vec(),
                                }
                            } else if let ThreadState::MultiRx {
                                expected_pci, buf, ..
                            } = &mut state
                            {
                                if packet_id == *expected_pci {
...
impl<T: VwApplicationProtocol> PayloadChannel for VwTransport2Channel<T> {
    fn open(&mut self) -> ChannelResult<()> {
        if let Some((tx, rx)) = self.tx_rx_ids.as_mut()
            && let ChannelState::Ready(can) = &mut self.can
        {
            can.close()?;
                ...
                if let Ok(packets) = can.read_packets(1000, 100) {
                    if let Some(bcast_resp) = packets.iter().find(|x| x.get_address() == rx_id) {
                        ...
                        if data[1] == 0xD0 {
                            ...
                            if can_tx_id != *tx {
                                log::warn!(
                                    "CAN Tx ID negotiated (0x{:04X}) is not the same as user requested (0x{:04X}). Overriding",
                                    can_tx_id,
                                    *tx
                                )
                            }
                            *tx = can_tx_id;
                            ...
                            return Err(ChannelError::Other(
                                "ECU Reject. Application type not supported".into(),
                            ));
                        } else if data[1] == 0xD7 {
                            return Err(ChannelError::Other(
                                "ECU Reject. Application type temporarily not supported".into(),
                            ));
                        } else if data[1] == 0xD8 {
                            return Err(ChannelError::Other(
                                "ECU Reject. Temporarily no resources are free".into(),
                            ));
                        } else {
                            // Negative response
            ...
            can.clear_rx_buffer()?;
            can.write_packets(vec![frame], 100)?;
            if let Some(config_resp) = can
                .read_packets(1000, 100)?
                .iter()
                .find(|x| x.get_address() == (*rx) as u32)
            {
                ...
                if data[0] == 0xA1 {
                    ...
                    log::debug!(
                        "ECU configuration reply: BS: {}, Ack timeout: {:?}, ST_MIN: {:?}",
                        bs,
                        ack_timeout,
                        st_min
                    );
