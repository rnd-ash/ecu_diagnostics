...
use std::{borrow::BorrowMut, ptr, sync::Arc, time::Duration};
...
impl PduDrv {
    ...
    pub fn load_lib(path: String) -> Result<PduDrv, libloading::Error> {
        ...
        unsafe {
            ...
            let get_last_error_fn = *lib
                .get::<PduGetListErrorFn>(b"PDUGetLastError\0")?
                .into_raw();
            let get_resources_status_fn = *lib
                .get::<PduGetResourceStatusFn>(b"PDUGetResourceStatus\0")?
                .into_raw();
            let create_cll_fn = *lib
                .get::<PduCreateComLogicalLinkFn>(b"PDUCreateComLogicalLink\0")?
                .into_raw();
            let destroy_cll_fn = *lib
                .get::<PduDestroyComLogicalLinkFn>(b"PDUDestroyComLogicalLink\0")?
                .into_raw();
            ...
            let lock_resource_fn = *lib
                .get::<PduLockResourceFn>(b"PDULockResource\0")?
                .into_raw();
            let unlock_resource_fn = *lib
                .get::<PduUnlockResourceFn>(b"PDUUnlockResource\0")?
                .into_raw();
            ...
            let start_cp_fn = *lib
                .get::<PduStartComPrimitiveFn>(b"PDUStartComPrimitive\0")?
                .into_raw();
            let cancel_cp_fn = *lib
                .get::<PduCancelComPrimitiveFn>(b"PDUCancelComPrimitive\0")?
                .into_raw();
            let get_evt_item_fn = *lib
                .get::<PduGetEventItemFn>(b"PDUGetEventItem\0")?
                .into_raw();
            let destroy_item_fn = *lib.get::<PduDestroyItemFn>(b"PDUDestroyItem\0")?.into_raw();
            let register_callback_fn = *lib
                .get::<PduRegisterCallbackFn>(b"PDURegisterEventCallback\0")?
                .into_raw();
            let get_obj_id_fn = *lib.get::<PduGetObjectIdFn>(b"PDUGetObjectId\0")?.into_raw();
            let get_module_ids_fn = *lib
                .get::<PduGetModuleIdsFn>(b"PDUGetModuleIds\0")?
                .into_raw();
            let get_res_ids_fn = *lib
                .get::<PduGetResourceIdsFn>(b"PDUGetResourceIds\0")?
                .into_raw();
            let get_conflicting_res_fn = *lib
                .get::<PduGetConflictingResourcesFn>(b"PDUGetConflictingResources\0")?
                .into_raw();
            let get_unique_resp_id_table_fn = *lib
                .get::<PduGetUniqueRespIdTableFn>(b"PDUGetUniqueRespIdTable\0")?
                .into_raw();
            let set_unqiue_resp_id_table_fn = *lib
                .get::<PduSetUniqueRespIdTableFn>(b"PDUSetUniqueRespIdTable\0")?
                .into_raw();
            let module_connect_fn = *lib
                .get::<PduModuleConnectFn>(b"PDUModuleConnect\0")?
                .into_raw();
            let module_disconnect_fn = *lib
                .get::<PduModuleDisconnectFn>(b"PDUModuleDisconnect\0")?
                .into_raw();
            let get_timestamp_fn = *lib
                .get::<PduGetTimestampFn>(b"PDUGetTimestamp\0")?
                .into_raw();
...
impl PduDrv {
    ...
    pub fn get_status(
        &mut self,
        vci_handle: Option<u32>,
        com_logical_link_handle: Option<u32>,
        com_primitive: u32,
    ) -> PDUResult<(PduStatus, u32, Option<ExtraInfo>)> {
        ...
        match (&self.get_status_fn)(
            ...
            PduError::StatusNoError => {
                ...
                Ok((status, timestamp, extra_info))
            }
            e => Err(e),
        }
    }
    ...
    pub fn get_last_error(
        &mut self,
        vci_handle: u32,
        com_logical_link_handle: u32,
        com_primitive_handle: Option<u32>,
    ) -> PDUResult<(PduErrorEvt, u32, Option<ExtraInfo>)> {
        ...
        match (&self.get_last_error_fn)(
            vci_handle,
            com_logical_link_handle,
            &mut event,
            &mut cph,
            &mut timestamp,
            &mut extra_info_ptr,
    ...
    pub fn create_com_logical_link(
        &mut self,
        vci_handle: u32,
        resource_data: &mut RscData,
        resource_id: u32,
    ) -> PDUResult<(u32, FlagData)> {
        let mut cll_id: u32 = 0;
        let mut cll_flag: FlagData = unsafe { std::mem::zeroed() };
        match (&self.create_cll_fn)(
            vci_handle,
            resource_data,
            resource_id,
            ptr::null_mut(),
    ...
    pub fn start_com_primitive(
        ...
        cp_type: PduCopt,
        data: &mut [u8],
        ctrl: &mut CopCtrlData,
    ) -> PDUResult<u32> {
        let data_ptr: *mut u8 = if data.len() == 0 {
            ptr::null_mut()
        } else {
            data.as_mut_ptr()
        };
    ...
    pub fn get_event_item(
        &mut self,
        vci_handle: Option<u32>,
        logical_link_handle: Option<u32>,
        event_data_prefilled: EventItem,
    ) -> PDUResult<Option<EventItem>> {
        if vci_handle.is_none() && logical_link_handle.is_some() {
            // Invalid input state
            return Err(PduError::InvalidParameters);
        }
        let vci = vci_handle.unwrap_or(PDU_HANDLE_UNDEF);
    ...
    pub fn get_object_id<T: Into<String>>(
        &mut self,
        object_type: PduObjt,
        name: T,
    ...
    pub fn get_resource_ids(&mut self, vci_handle: Option<u32>) -> PDUResult<(RscData, RscIdItem)> {
        ...
        match (&self.get_res_ids_fn)(vci, &mut data, list) {
            PduError::StatusNoError => Ok((data, unsafe { Box::from_raw(list).read() })),
            e => Err(e),
        }
    }
    ...
    pub fn get_conflicting_resources(
        ...
        match (&self.get_conflicting_res_fn)(resource_id, &mut m_list, list) {
            PduError::StatusNoError => Ok(unsafe { Box::from_raw(list).read() }),
            e => Err(e),
        }
    }
    ...
    pub fn get_unqiue_response_id_table(
        &mut self,
        vci_handle: u32,
        com_logical_link_handle: u32,
    ) -> PDUResult<UniqueRespIdTableItem> {
        ...
        match (&self.get_unique_resp_id_table_fn)(vci_handle, com_logical_link_handle, table) {
            PduError::StatusNoError => Ok(unsafe { Box::from_raw(table).read() }),
            e => Err(e),
        }
    }
    ...
    pub fn set_unqiue_response_id_table(
        &mut self,
        vci_handle: u32,
        com_logical_link_handle: u32,
        unique_resp_table: &mut UniqueRespIdTableItem,
    ) -> PDUResult<()> {
        match (&self.set_unqiue_resp_id_table_fn)(
            vci_handle,
            com_logical_link_handle,
            unique_resp_table,
        ) {
