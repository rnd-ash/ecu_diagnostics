...
impl Into<u8> for SecurityOperation {
    fn into(self) -> u8 {
        match self {
...
pub fn request_seed(server: &mut UdsDiagnosticServer) -> DiagServerResult<Vec<u8>> {
    let mut resp = server.execute_command_with_response(UDSCommand::SecurityAccess, &[SecurityOperation::RequestSeed.into()])?;
...
pub fn send_key(server: &mut UdsDiagnosticServer, key: &[u8]) -> DiagServerResult<()> {
    ...
    server.execute_command_with_response(UDSCommand::SecurityAccess, &payload).map(|_| ())
}