...
use ecu_diagnostics::{channel::{CanFrame, CanChannel, PacketChannel, ChannelError, IsoTPChannel, IsoTPSettings, PayloadChannel}, hardware::software_isotp::SoftwareIsoTpChannel};
...
impl EmuCanChannel {
    pub fn new(sender: mpsc::Sender<CanFrame>, receiver: mpsc::Receiver<CanFrame>) -> Self {
        Self {
...
impl CanChannel for EmuCanChannel {
    fn set_can_cfg(&mut self, baud: u32, use_extended: bool) -> ecu_diagnostics::channel::ChannelResult<()> {
        Ok(())
    }
}
...
impl PacketChannel<CanFrame> for EmuCanChannel {
    ...
    fn write_packets(&mut self, packets: Vec<CanFrame>, timeout_ms: u32) -> ecu_diagnostics::channel::ChannelResult<()> {
        for p in packets {
            log::debug!("Out -> {p:02X?}");
    ...
    fn read_packets(&mut self, max: usize, timeout_ms: u32) -> ecu_diagnostics::channel::ChannelResult<Vec<CanFrame>> {
        let mut read_packets = vec![];
        let start = Instant::now();
        loop {
            let res = self.in_queue.try_recv().map_err(|e| ChannelError::BufferEmpty);
            match res {
                Ok(f) => {
                    log::debug!("In -> {f:02X?}");
                    read_packets.push(f);
...
fn setup(bs: u8, stmin: u8, padding: bool, ext_address: Option<(u8, u8)>, ecu1_addr: u32, ecu2_addr: u32) -> (SoftwareIsoTpChannel, SoftwareIsoTpChannel) {
    let (ecu1tx, ecu1rx) = mpsc::channel::<CanFrame>();
    let (ecu2tx, ecu2rx) = mpsc::channel::<CanFrame>();
    let ecu1 = Box::new(EmuCanChannel::new(ecu1tx, ecu1rx));
    let ecu2 = Box::new(EmuCanChannel::new(ecu2tx, ecu2rx));
